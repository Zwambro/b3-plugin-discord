...
#  03.11.2019 - v2.0.1 - Zwmabro
#  - adding reason to the report syntax
#  03.03.2020 - v2.0.2 - Zwambro
#  - adding player @ID
...
class DiscordEmbed:
    ...
    def push(self, *arg):
        ...
        if self.mapview:
            embed["thumbnail"]['url'] = self.mapview
        if self.desc:
...
class DiscordPlugin(b3.plugin.Plugin):
    ...
    def cmd_clean(self, data, client=None, cmd=None):
        if not data:
            ...
            player = str(self._adminPlugin.findClientPrompt(
            ...
            if cleanplayer in self.reportedplayers:
                ...
                embed.set_mapview(
                    'https://www.iconsdb.com/icons/download/green/checkmark-16.png')
                embed.set_desc("%s has been checked by %s" % (
                    cleanplayer, self.stripColors(client.exactName)))
                embed.set_footnote()
                embed.post()
